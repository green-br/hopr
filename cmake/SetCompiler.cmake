...
MARK_AS_ADVANCED(FORCE C_PATH Fortran_PATH)
...
IF (CMAKE_FQDN_HOST MATCHES "sng\.lrz\.de$")
  ...
  IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU" OR CMAKE_Fortran_COMPILER_ID MATCHES "Flang")
    SET(HOPR_INSTRUCION "-march=skylake-avx512 -mtune=skylake-avx512" CACHE STRING "Compiler optimization options")
  ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "Intel")
    SET(HOPR_INSTRUCION "-xSKYLAKE-AVX512" CACHE STRING "Compiler optimization options")
...
ELSEIF(CMAKE_FQDN_HOST MATCHES "\.can$")
  ...
  IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU")
    SET(HOPR_INSTRUCION "-march=znver3 -mtune=znver3" CACHE STRING "Compiler optimization options")
  ELSE()
...
ELSEIF(CMAKE_FQDN_HOST MATCHES "^(prandtl|grafik.*)\.iag\.uni\-stuttgart\.de")
  ...
  SET(HOPR_INSTRUCION "-march=native -mtune=native" CACHE STRING "Compiler optimization options")
...
ELSEIF(CMAKE_HOST_APPLE)
  ...
  IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU" OR CMAKE_Fortran_COMPILER_ID MATCHES "Flang" OR CMAKE_Fortran_COMPILER_ID MATCHES "Cray")
    SET(HOPR_INSTRUCION "-march=native" CACHE STRING "Compiler optimization options")
  ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "Intel")
    SET(HOPR_INSTRUCION "-xHost" CACHE STRING "Compiler optimization options")
  ENDIF()
...
ELSE()
  ...
  IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU" OR CMAKE_Fortran_COMPILER_ID MATCHES "Flang" OR CMAKE_Fortran_COMPILER_ID MATCHES "Cray")
    SET(HOPR_INSTRUCION "-march=native -mtune=native" CACHE STRING "Compiler optimization options")
  ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "Intel")
    SET(HOPR_INSTRUCTION "-xHost" CACHE STRING "Compiler optimization options")
  ENDIF()
ENDIF()
...
IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU")
  ...
  SET (CMAKE_Fortran_FLAGS_RELWITHDEBINFO "${CMAKE_Fortran_FLAGS} -g  -O3 ${HOPR_INSTRUCTION} -finline-functions -fstack-arrays -ffpe-trap=invalid,zero,overflow -fbacktrace")
  ...
  ELSE()
    ...
    SET (HOPR_COMPILE_FLAGS "${NINJA_COLOR_DIAGNOSTICS} ")
  ENDIF()
...
ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "Flang")
  ...
  IF (CMAKE_Fortran_COMPILER_VERSION VERSION_GREATER_EQUAL "20.0.0")
    ...
    ENDIF()
    SET (CMAKE_Fortran_FLAGS_RELEASE        "${CMAKE_Fortran_FLAGS}     -O3 ${HOPR_INSTRUCTION} ")
    SET (CMAKE_Fortran_FLAGS_RELWITHDEBINFO "${CMAKE_Fortran_FLAGS}     -O3 ${HOPR_INSTRUCTION} -ffpe-trap=invalid,zero,overflow -fbacktrace")
    SET (CMAKE_Fortran_FLAGS_PROFILE        "${CMAKE_Fortran_FLAGS} -pg -O3 ${HOPR_INSTRUCTION} ")
  ...
  ELSE()
    ...
    SET (CMAKE_Fortran_FLAGS_RELWITHDEBINFO "${CMAKE_Fortran_FLAGS}     -O3 ${HOPR_INSTRUCTION} -finline-functions -ffpe-trap=invalid,zero,overflow -fbacktrace")
...
ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "Intel")
  ...
  ENDIF()
  SET (CMAKE_Fortran_FLAGS_RELEASE        "${CMAKE_Fortran_FLAGS}    -O3 ${HOPR_INSTRUCTION} -qopt-report0 -qopt-report-phase=vec -no-prec-div")
  SET (CMAKE_Fortran_FLAGS_RELWITHDEBINFO "${CMAKE_Fortran_FLAGS}    -O3 ${HOPR_INSTRUCTION} -qopt-report0 -qopt-report-phase=vec -no-prec-div -fpe0 -traceback")
  SET (CMAKE_Fortran_FLAGS_PROFILE        "${CMAKE_Fortran_FLAGS} -p -O3 ${HOPR_INSTRUCTION} -qopt-report0 -qopt-report-phase=vec -no-prec-div")
...
# =========================================================================
MARK_AS_ADVANCED(FORCE HOPR_INSTRUCION)
